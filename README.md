...
Chart extraction consists of extracting quantitative data from graphical figures that contain essentially performance-related information. These charts typically contain performance indicators (e.g., conversion, selectivity, yield, stability, activity) plotted against variables such as temperature, time, pressure, or composition for different catalysts or experimental conditions.

You can run it using the following command:

```bash
xlmexlab-image2data image_folder_path output_file_path.json --vlm_model_name <model_name>
```

The input should be a folder containing the chart images (.png, .jpeg, or .tiff).

The output is a .json file containing a nested dictionary with the structured information extracted from each figure.



Chart extraction relies on:
###### Prompt Template
A prompt template defining how the model is instructed to interpret the chart. If no ```--prompt_template_path``` is provided, the system automatically selects a template from the internal ```TEMPLATE_REGISTRY``` based on the provided ```--vlm_model_name.```
###### Prompt Schema
A prompt schema defining the expected structured output format. To customize extraction for different types of charts, you can provide a schema file using ```--prompt_schema_path path_to_schema.json```


**Additional Options**

```--vlm_model_name```
Name of the Vision-Language Model used to process the figures.

```--vlm_model_parameters_path```
Optional path to model parameters (used in case of vLLM inference).

```--scale```
Scale factor to reduce image resolution before processing (e.g., 0.5 for 50% resolution). 


#### Evaluation

Evaluation consists of comparing the extracted structured data generated by ```xlmexlab-image2data``` against a ground-truth reference file.

The goal of the evaluation step is to measure how accurately the system extracts variable names and numerical values.

You can run the evaluation using:

```bash
xlmexlab-evaluate predictions.json ground_truth.json output_metrics.json
```

Where:

- ```predictions.json``` is the file generated by the extraction pipeline.

- ```ground_truth.json``` is the reference file.

- ```output_metrics.json``` is the file where evaluation results will be written.

(note: ```predictions.json``` and ```ground_truth.json``` should follow the same structure as the one proposed on ```./Resources/schemas/image_extraction_schema.json``` file.)

**Additional Options**
```--label-threshold'--series-threshold```
Threshold for matching labels and series, respectively, based on the Levenshtein Ratio.

```--point-distance-threshold```
Threshold for matching points based on Euclidean distance.


### Run Inference with a Model Using an Adapter

To run inference with a LoRA fine-tuned model, you must provide a .json file containing the required model parameters, the folder with the adapter weights and the base model name.

**Note: Currently, this procedure is only supported for the Qwen-2.5V-L-Instruct-7B model.**
